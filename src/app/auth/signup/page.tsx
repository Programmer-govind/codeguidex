...
import { Mail, ArrowRight, Code, Lock, User, Eye, EyeOff, Loader2 } from "lucide-react"
...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const result = await signup(displayName, email, password, role)
      if (result.success) {
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4">
      ...
      >
        <Card className="border-primary/20 bg-background/60 backdrop-blur-2xl shadow-[0_0_50px_rgba(79,70,229,0.15)] relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-cyan-400 via-purple-500 to-primary" />

          <CardHeader className="space-y-3 pb-6 text-center pt-8">
            <div className="mx-auto bg-purple-500/10 w-12 h-12 rounded-full flex items-center justify-center mb-2 ring-1 ring-purple-500/30">
              <Code className="h-6 w-6 text-purple-500" />
            </div>
            <CardTitle className="text-3xl font-bold tracking-tight">Join CodeGuideX</CardTitle>
            <CardDescription className="text-base">
              Create an account to connect with mentors and generate code.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-5">
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    required
                    disabled={isLoading}
                    className="pl-10 pr-10 h-11 bg-background/50 border-white/10 focus-visible:ring-primary"
                  />
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(["student", "mentor"] as const).map((r) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setRole(r)}
                      className={`h-10 rounded-lg border text-sm font-medium transition-all capitalize ${
                        role === r
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="w-full h-11 bg-gradient-to-r from-purple-600 to-primary hover:from-purple-600/90 hover:to-primary/90 text-white shadow-lg shadow-purple-600/25 mt-2"
              >
